...
def verify_js_array_loop_to_map(code: str, row: dict[str, Any]) -> VerifyResult:
    syntax_valid = parses_as_js(code)
    uses_map = ".map(" in code or ".map (" in code
    passed = False
    test_results = []
    if syntax_valid and uses_map:
        passed, test_results = run_contract_tests(
            code,
